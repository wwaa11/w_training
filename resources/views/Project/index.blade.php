...
@section("content")
    ...
    @else
        <div class="mt-[20%] flex w-full flex-col">
            <div class="m-auto w-full rounded bg-[#c1dccd] p-3 text-center shadow-lg md:w-3/4 md:p-6 lg:w-1/2">
                <form id="changePassword" action="{{ env("APP_URL") }}/changePassword" method="POST">
                    @csrf
                    <div class="mb-3 text-2xl font-bold text-red-600">กรุณาเปลี่ยนรหัสผ่าน</div>
                    ...
                    <div>
                        <button class="w-full cursor-pointer rounded bg-amber-400 p-3 font-bold" type="button" onclick="changePassword()">เปลี่ยนรหัสผ่าน</button>
                    </div>
                </form>
            </div>
        </div>
