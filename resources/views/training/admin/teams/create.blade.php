    ...
    <div class="container mx-auto max-w-2xl py-10">
        ...
        <div class="rounded-lg border bg-white shadow-sm">
            <div class="border-b border-gray-200 px-8 py-6">
                ...
                <p class="mt-1 text-sm text-gray-600">Add a new training group</p>
            </div>

            <form class="p-8" method="POST" action="{{ route("training.admin.teams.store") }}">
                @csrf

                <div class="mb-6">
                    <label class="mb-2 block text-sm font-medium text-gray-700" for="name">Group Name</label>
                    <input class="@error("name") border-red-500 @enderror w-full rounded-md border border-gray-300 px-3 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-500" id="name" type="text" name="name" value="{{ old("name") }}" placeholder="Enter team name" required>
                    @error("name")
