    ...
    <div class="container mx-auto max-w-4xl py-10">
        ...
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-purple-800">
                <i class="fa-solid fa-user-edit mr-2"></i>Edit Teacher: {{ $teacher->name }}
            </h1>
        ...
        <div class="rounded-lg border bg-white p-6 shadow-sm">
            <form method="POST" action="{{ route("training.admin.teachers.update", $teacher->id) }}">
                @csrf
                <div class="grid grid-cols-1 gap-6 md:grid-cols-2">
                    <input type="hidden" name="team_id" value='{{ $teacher->team->id }}'>
                    <!-- Teacher Name -->
                    <div class="md:col-span-2">
                        <label class="mb-2 block text-sm font-medium text-gray-700" for="name">
                            Teacher Name <span class="text-red-500">*</span>
                        </label>
                        <input class="@error("name") border-red-500 @enderror w-full rounded-md border border-gray-300 px-3 py-2 focus:border-transparent focus:outline-none focus:ring-2 focus:ring-purple-500" id="name" type="text" name="name" value="{{ old("name", $teacher->name) }}" placeholder="Enter teacher name" required>
                        @error("name")
                ...
                <div class="mt-8 flex items-center justify-end space-x-4">
                    <a class="rounded-md bg-gray-100 px-4 py-2 text-gray-700 transition-colors hover:bg-gray-200" href="{{ route("training.admin.teams.teachers", $teacher->team_id) }}">
                        Cancel
                    </a>
