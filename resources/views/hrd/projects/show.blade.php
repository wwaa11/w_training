        ...
        <div class="grid grid-cols-1 gap-8 lg:grid-cols-3">
            ...
            <div class="lg:col-span-2">
                ...
                <div class="mb-6 rounded-lg bg-white p-6 shadow-md">
                    <div class="mb-4 flex items-start justify-between">
                        @if ($registrationData["statusBadge"])
                            <span class="bg-{{ $registrationData["statusBadge"]["type"] }}-100 text-{{ $registrationData["statusBadge"]["type"] }}-800 inline-flex items-center rounded-full px-3 py-1 text-sm font-medium">
                    ...
                    <h1 class="mb-4 text-3xl font-bold text-gray-900">{{ $project->project_name }}</h1>

                    @if ($project->project_detail)
                        <div class="prose prose-gray max-w-none">
                            <p class="leading-relaxed text-gray-700">{{ $project->project_detail }}</p>
                        </div>
                    @endif
                </div>
                        ...
                        @foreach ($project->dates->where("date_delete", false) as $date)
                            <div class="rounded-lg border border-gray-200 p-4">
                                <div class="mb-3 flex items-center justify-between">
                                    <h3 class="text-lg font-medium text-gray-900">{{ $date->date_title }}</h3>
                                    <span class="text-sm text-gray-500">{{ $date->date_datetime->format("l, d M Y") }}</span>
                                </div>
                                                ...
                                                <div class="{{ $timeState["hasAttended"] ? "bg-blue-50 border-blue-200" : ($timeState["userRegistered"] ? "bg-green-50 border-green-200" : "bg-gray-50") }} rounded-lg border border-gray-100 p-3">
                                                    <div class="mb-2 flex items-center justify-between">
                                                        ...
                                                        @if ($time->time_limit && $project->project_type !== "attendance")
                                                            ...
                                                            @endphp
                                                            <span class="{{ $available > 0 ? "bg-green-100 text-green-800" : "bg-red-100 text-red-800" }} rounded-full px-2 py-1 text-xs">
                ...
                @if ($registrationData["showRegisterForm"])
                    <div class="mb-6 rounded-lg bg-white p-6 shadow-md" id="registration">
                        @if ($project->isFull())
                        ...
                        @else
                            ...
                            <form id="registrationForm" action="{{ route("hrd.projects.register.store", $project->id) }}" method="POST">
                                ...
                                <div class="mt-6 flex justify-end">
                                    <button class="inline-flex items-center rounded-md border border-transparent bg-blue-600 px-6 py-3 text-base font-medium text-white shadow-sm hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2" type="submit">
            ...
            <div class="lg:col-span-1">
                <!-- Registration Info -->
                <div class="mb-6 rounded-lg bg-white p-6 shadow-md">
                    <h3 class="mb-4 text-lg font-semibold text-gray-900">ข้อมูลการลงทะเบียน</h3>
